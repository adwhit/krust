...
impl Handle {
    pub fn connect(host: &str, port: i32, username: &str) -> Result<Handle, Box<Error>> {
        ...
        let handle = match unsafe { kbindings::khpu(chost, port, cuser) } {
    ...
    pub fn query(&self, query: &str) -> Result<KOwned, Box<Error>> {
        ...
        let kptr = unsafe { kbindings::k(self.handle, cquery, kvoid()) };
        if kptr.is_null() {
    ...
    pub fn close(&self) {
        unsafe { kbindings::kclose(self.handle) };
    }
}

pub fn serialize(k: &KOwned) -> KResult<KOwned> {
    let ser = unsafe { &*kbindings::b9(3, k.0) };
